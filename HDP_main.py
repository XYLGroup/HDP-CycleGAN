...
def config():
    ...
    parser.add_argument("--dataset_name", type=str, default="satellite", help="satellite")
    ...
    parser.add_argument("--meta_update", type=int, default=7)
    return parser.parse_args()


####### feature-consistency loss #######
def feature_loss(real_A_img, recov_A_img, sift):
    ...
    if len(des_recov_A[:, 1]) < 20:
        des_recov_A = np.vstack((des_recov_A, np.zeros_like(des_real_A)))
    REAL = Variable(torch.tensor(des_real_A[:20]).unsqueeze(0), requires_grad=False).type(Tensor)
    RECOV = Variable(torch.tensor(des_recov_A[:20]).unsqueeze(0), requires_grad=False).type(Tensor)
...
def classification_loss(ISAR_image, recov_ISAR_img):
...
dataloader = DataLoader(ImageDataset(root=opt.path + r'\%s' % opt.dataset_name, transforms_=transforms_,
                                     unaligned=True), batch_size=opt.batch_size, shuffle=True, num_workers=opt.n_cpu)
# testing data
...
for epoch in range(opt.epoch, opt.n_epochs):
    for step, batch in enumerate(dataloader):
        ...
        if (Batch + 1) % opt.meta_update == 0 and Batch != 0:
            # feature-consistency loss
            # classification-consistency loss
            loss_feature += feature_loss(ISAR_img_np, recov_ISAR_np, sift=sift)
            loss_class += classification_loss(ISAR_img.detach(), recov_ISAR.detach())
            # total loss
